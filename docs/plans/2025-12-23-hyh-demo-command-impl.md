# `hyh demo` Command Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Port the interactive demo from `demo.sh` to a native `hyh demo` Python command.

**Architecture:** Single `demo.py` module with output helpers and step functions. Client.py adds subparser and dispatches to `demo.run()`. Demo creates isolated temp directory, runs all 12 steps interactively, cleans up on exit.

**Tech Stack:** Python stdlib only (subprocess, tempfile, shutil, os, pathlib). ANSI escape codes for colors.

---

## Task 1: Create demo.py with Output Helpers
...
Create test file with tests for formatting functions:
...
Run: `cd /Users/pedroproenca/Documents/Projects/hyh/.worktrees/demo-command && uv run pytest tests/hyh/test_demo.py -v`

Expected: FAIL with ModuleNotFoundError (demo module doesn't exist)

**Step 3: Create demo.py with output helpers**
...
```bash
cd /Users/pedroproenca/Documents/Projects/hyh/.worktrees/demo-command
git add src/hyh/demo.py tests/hyh/test_demo.py
git commit -m "feat(demo): add output helper functions with tests"
```

---

## Task 2: Add Cleanup and Run Skeleton

**Files:**
- Modify: `src/hyh/demo.py`
- Test: `tests/hyh/test_demo.py`

**Step 1: Write test for cleanup isolation**
...
Run: `cd /Users/pedroproenca/Documents/Projects/hyh/.worktrees/demo-command && uv run pytest tests/hyh/test_demo.py::test_cleanup_removes_temp_directory tests/hyh/test_demo.py::test_run_restores_cwd -v`

Expected: FAIL (cleanup and run don't exist)

**Step 3: Add cleanup and run functions**

Append to `src/hyh/demo.py`:
...
cd /Users/pedroproenca/Documents/Projects/hyh/.worktrees/demo-command
git add src/hyh/demo.py tests/hyh/test_demo.py
git commit -m "feat(demo): add cleanup and run skeleton"
```

---

## Task 3: Integrate with client.py

**Files:**
- Modify: `src/hyh/client.py`

**Step 1: Add import at top of client.py**
...
**Step 2: Add subparser in main()**

Find the line `subparsers.add_parser("worker-id"` and add after it:

```python
    subparsers.add_parser("demo", help="Interactive tour of hyh features")
```

**Step 3: Add case in match statement**

Find the `case "worker-id":` block and add after it:
...
Run: `cd /Users/pedroproenca/Documents/Projects/hyh/.worktrees/demo-command && uv run hyh demo --help`

Expected: Shows "Interactive tour of hyh features"

Run: `cd /Users/pedroproenca/Documents/Projects/hyh/.worktrees/demo-command && uv run hyh --help`

Expected: Shows "demo" in the list of commands

**Step 5: Run all tests**

Run: `cd /Users/pedroproenca/Documents/Projects/hyh/.worktrees/demo-command && uv run pytest -q`
...
git commit -m "feat(demo): integrate demo command into CLI"
```

---

## Task 4: Implement Step 1 (Setup)

**Files:**
- Modify: `src/hyh/demo.py`

**Step 1: Add sample workflow JSON constant**
...
**Step 2: Add step_01_intro function**
...
def step_01_intro() -> None:
    ...
    print(f"  {BOLD}Harness{NC} is a thread-safe state management daemon for dev workflows.")
    print()
    print("  It solves three problems:")
    print()
    print(f"    {GREEN}1.{NC} {BOLD}Task Coordination{NC} - Multiple workers claim/complete tasks from a DAG")
    print(f"    {GREEN}2.{NC} {BOLD}Git Safety{NC} - Mutex prevents parallel git operations corrupting .git/index")
    print(f"    {GREEN}3.{NC} {BOLD}Crash Recovery{NC} - Atomic writes ensure state survives power failures")
    print()
...
**Step 3: Add step_02_setup function**
...
**Step 4: Update _run_all_steps to call step functions**
...
Run: `cd /Users/pedroproenca/Documents/Projects/hyh/.worktrees/demo-command && uv run hyh demo`

Press Enter twice to go through intro and setup. Then Ctrl+C.

Expected: See welcome message, then setup with ASCII DAG.
...
git commit -m "feat(demo): implement intro and setup steps"
```

---

## Task 5: Implement Steps 2-4 (Worker Identity, Plan Import, Basic Commands)

**Files:**
- Modify: `src/hyh/demo.py`

**Step 1: Add sample LLM plan constant**

Add after SAMPLE_WORKFLOW_JSON:
...
**Step 2: Add step_03_worker_identity function**
...
**Step 3: Add step_04_plan_import function**
...
**Step 4: Add step_05_basic_commands function**
...
cd /Users/pedroproenca/Documents/Projects/hyh/.worktrees/demo-command
git add src/hyh/demo.py
git commit -m "feat(demo): implement worker identity, plan import, basic commands"
```

---

## Task 6: Implement Steps 5-6 (Status Dashboard, Task Workflow)

**Files:**
- Modify: `src/hyh/demo.py`

**Step 1: Add step_06_status_dashboard function**
...
**Step 2: Add step_07_task_workflow function**
...
'""")
    ...
    result = subprocess.run(
        ["hyh", "task", "claim"], capture_output=True, text=True
    )
    import json
    claim_data = json.loads(result.stdout)
    task_id = claim_data.get("task", {}).get("id", "auth-endpoints")
    print(f"    Claimed: {task_id}")
    print()

    print_command(f"hyh task complete --id {task_id}")
    subprocess.run(["hyh", "task", "complete", "--id", task_id], capture_output=True)
    print()
    ...
    claim_data = json.loads(result.stdout)
    task_id = claim_data.get("task", {}).get("id", "api-tests")
    print(f"    Claimed: {task_id}")
    print()
...
cd /Users/pedroproenca/Documents/Projects/hyh/.worktrees/demo-command
git add src/hyh/demo.py
git commit -m "feat(demo): implement status dashboard and task workflow"
```

---

## Task 7: Implement Steps 7-8 (Git Mutex, Hooks)

**Files:**
- Modify: `src/hyh/demo.py`

**Step 1: Add step_08_git_mutex function**
...
**Step 2: Add step_09_hooks function**
...
def step_09_hooks(demo_dir: Path) -> None:
  ...
  "tasks": {
    ...
    # Complete the task
    subprocess.run(["hyh", "task", "claim"], capture_output=True)
    subprocess.run(
        ["hyh", "task", "complete", "--id", "incomplete-task"], capture_output=True
    )
    ...
    subprocess.run(
        ["hyh", "update-state", "--field", "last_commit", current_head],
        capture_output=True,
    )
...
cd /Users/pedroproenca/Documents/Projects/hyh/.worktrees/demo-command
git add src/hyh/demo.py
git commit -m "feat(demo): implement git mutex and hooks steps"
```

---

## Task 8: Implement Steps 9-11 (Multi-Project, Exec, State Update)

**Files:**
- Modify: `src/hyh/demo.py`

**Step 1: Add step_10_multi_project function**
...
**Step 2: Add step_11_exec function**
...
**Step 3: Add step_12_state_update function**
...
**Step 4: Add hashlib import at top**
...
cd /Users/pedroproenca/Documents/Projects/hyh/.worktrees/demo-command
git add src/hyh/demo.py
git commit -m "feat(demo): implement multi-project, exec, and state update steps"
```

---

## Task 9: Implement Step 12 (Architecture Overview) and Recap

**Files:**
- Modify: `src/hyh/demo.py`

**Step 1: Add step_13_architecture function**
...
**Step 2: Add step_14_recap function**
...
**Step 3: Add step_15_next_steps function**
...
cd /Users/pedroproenca/Documents/Projects/hyh/.worktrees/demo-command
git add src/hyh/demo.py
git commit -m "feat(demo): implement architecture overview and recap"
```

---

## Task 10: Add Deprecation Note to demo.sh

**Files:**
- Modify: `demo.sh`

**Step 1: Add deprecation note at top of demo.sh**

After the shebang and before the first comment, add:
...
cd /Users/pedroproenca/Documents/Projects/hyh/.worktrees/demo-command
git add demo.sh
git commit -m "docs: add deprecation note to demo.sh"
```
...
Run: `cd /Users/pedroproenca/Documents/Projects/hyh/.worktrees/demo-command && make check`
...
Walk through a few steps, verify output looks correct, Ctrl+C to exit.

Expected: Demo runs, shows colored output, cleans up on exit.

**Step 4: Verify demo command appears in help**

Run: `cd /Users/pedroproenca/Documents/Projects/hyh/.worktrees/demo-command && uv run hyh --help`

Expected: `demo` appears in command list with description "Interactive tour of hyh features"
...
|------|-------------|---------|
| 1 | Output helpers + tests | `feat(demo): add output helper functions with tests` |
| 2 | Cleanup and run skeleton | `feat(demo): add cleanup and run skeleton` |
| 3 | Client.py integration | `feat(demo): integrate demo command into CLI` |
| 4 | Step 1 (intro + setup) | `feat(demo): implement intro and setup steps` |
| 5 | Steps 2-4 | `feat(demo): implement worker identity, plan import, basic commands` |
| 6 | Steps 5-6 | `feat(demo): implement status dashboard and task workflow` |
| 7 | Steps 7-8 | `feat(demo): implement git mutex and hooks steps` |
| 8 | Steps 9-11 | `feat(demo): implement multi-project, exec, and state update steps` |
| 9 | Step 12 + recap | `feat(demo): implement architecture overview and recap` |
| 10 | Deprecate demo.sh | `docs: add deprecation note to demo.sh` |
| 11 | Final testing | (no commit) |

Total: 10 commits, ~600 lines of Python code
