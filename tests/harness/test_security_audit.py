...
class TestSocketPathOverride:
    ...
    Vulnerability: HARNESS_SOCKET env var can redirect IPC to arbitrary path.
    """

    def test_socket_path_to_devnull_accepted(self) -> None:
        """Setting HARNESS_SOCKET=/dev/null should be rejected or handled.

        This documents the vulnerability - /dev/null causes DoS.
        """
        from harness.client import get_socket_path

        with patch.dict(os.environ, {"HARNESS_SOCKET": "/dev/null"}):
    ...
    def test_socket_path_with_null_bytes(self) -> None:
        ...
        with pytest.raises(ValueError, match="null byte"):
            os.environ["HARNESS_SOCKET"] = "/tmp/test\x00.sock"
